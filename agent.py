...
class Agent:
    ...
    def getActionsStr(self, actions):
        ...
        return ''.join(sorted(map(str, list(actions.values()))))
    ...
    def findAttribute(self, name, model=True):
        ...
        # todo Pedro check ignore parent's beliefs
                ...
                (name == 'beliefs' and 'ignore-parent-beliefs' in self.models[model] and
    ...
    def update_model(self, old_state, new_state, actions_idxs, model=True):
        ...
        elif old_belief_states not in self.models[model]['SE']:
            self.models[model]['SE'][old_belief_states] = {new_state: {}}
        elif new_state not in self.models[model]['SE'][old_belief_states]:
            self.models[model]['SE'][old_belief_states][new_state] = {}
        elif actions_idxs in self.models[model]['SE'][old_belief_states][new_state]:
        ...
        for old_belief in old_beliefs.domain():
            ...
            for new_belief_state in new_belief_states.domain():
                ...
                for key in list(new_belief_state.keys()):
                    if key in old_belief:
