...
class MoCA:
    ...
    def __init__(self, turnOrder):
		...
		self.world.defineState(child.name, 'Liking', float, lo=minMax['min'], hi=minMax['max'], description='Child liking level')
		child.setState('Liking', 0.8)

		self.world.defineState(child.name, 'KnowledgeIncrement', float, lo=minMax['min'], hi=minMax['max'], description='How much a working unit improve the child knowledge')
		child.setState('KnowledgeIncrement', 0.2)
		self.world.defineState(child.name, 'TotalKnowledge', float, lo=minMax['min'], hi=minMax['max'], description='Child total knowledge value')
		child.setState('TotalKnowledge', 0)
		self.world.defineState(child.name, 'FunIncrement', float, lo=minMax['min'], hi=minMax['max'], description='How much a working unit improve the child fun')
		child.setState('FunIncrement', 0.0)
		self.world.defineState(child.name, 'TotalFun', float, lo=minMax['min'], hi=minMax['max'], description='Child fun value')
		...
		self.world.defineState(child.name, 'BestInClassThreshold', float, lo=minMax['min'], hi=minMax['max'], description='Threshold of knowledge needed to be the best in class')
		...
		self.world.defineState(greta.name, 'Power', float, lo=minMax['min'], hi=minMax['max'], description='Teacher Power value')
...
##		greta.addAction({'verb': 'Encourage'})
        ...
        # Dynamics of actions
            ...
            #Child Actions
                ...
                tree = makeTree({'if': greaterThanRow(stateKey(child.name,'TotalKnowledge'),stateKey(child.name,'BestInClassThreshold')),
